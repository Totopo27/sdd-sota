"""CSV / DataFrame fetcher for local datasets and testing."""
...
class CSVFetcher(Fetcher):
    """Fetch papers from a local CSV file.
    ...
    def fetch(self, collection_names: list[str] | None = None) -> pd.DataFrame:
        """Read CSV and normalize to litreview format."""
        ...
        if "Title" not in df.columns:
            raise ValueError("CSV must contain a 'Title' or 'title' column.")
        if "Abstract Note" not in df.columns:
            raise ValueError("CSV must contain an 'Abstract Note' or 'abstract' column.")
